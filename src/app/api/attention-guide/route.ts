          ...
          <tr>
            <td style="background-color: #141416; padding: 32px 40px; border-radius: 16px 16px 0 0;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                ...
                <tr><td style="padding-top: 24px;"><p style="margin: 0; font-size: 11px; letter-spacing: 2px; text-transform: uppercase; color: #e5ff21; font-weight: 600;">Attention is New Gold</p></td></tr>
          ...
          <tr>
            <td style="background-color: #ffffff; padding: 40px;">
              <p style="margin: 0 0 16px; font-size: 15px; line-height: 1.7; color: #555;">
                Thanks for downloading <strong>Attention is New Gold</strong>. 6 pages of practical frameworks for making your brand and stories stand out in the most distracted era in human history.
              </p>
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" style="margin-bottom: 32px;">
                <tr>
                  <td style="background-color: #141416; border-radius: 100px;">
                    <a href="https://smpl.as/downloads/attention-is-new-gold.pdf" style="display: inline-block; padding: 14px 32px; color: #ffffff; font-size: 14px; font-weight: 600; text-decoration: none;">Download the Guide</a>
                  </td>
                </tr>
              </table>
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 28px;">
                <tr>
                  <td style="padding: 24px; background-color: #f5f5f0; border-radius: 12px;">
                    ...
                    <p style="margin: 0 0 4px; font-size: 14px; color: #555;">1. Why attention matters more than ever</p>
                    <p style="margin: 0 0 4px; font-size: 14px; color: #555;">2. The cornerstones of brand value</p>
                    <p style="margin: 0 0 4px; font-size: 14px; color: #555;">3. The TRUTH storytelling framework</p>
                    <p style="margin: 0 0 4px; font-size: 14px; color: #555;">4. 8 behavioural science principles that win customers</p>
                    <p style="margin: 0 0 4px; font-size: 14px; color: #555;">5. 6 field lessons for founders</p>
                    <p style="margin: 0; font-size: 14px; color: #555;">6. Quick reference card</p>
                  </td>
                </tr>
              </table>
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 24px;">
                <tr>
                  <td style="padding: 24px; background-color: #e5ff21; border-radius: 12px;">
                    ...
                    <p style="margin: 0 0 16px; font-size: 13px; line-height: 1.6; color: #333;">Want help building a brand that cuts through? We'll talk through your situation, no strings attached.</p>
              ...
              <p style="margin: 0; font-size: 15px; color: #141416; font-weight: 500;">
                Go make some noise,<br>Mike &amp; the SmplCo Team
              </p>
            </td>
          </tr>
          <tr>
...
function notificationEmailHtml(name: string, email: string, company: string | undefined, biggestChallenge: string | undefined, wantsConsultation?: boolean) {
  return `
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>
        ...
        <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width: 560px; width: 100%;">
          <tr>
            <td style="background-color: #141416; padding: 32px 40px; border-radius: 16px 16px 0 0;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                <tr><td>${logoWhite}</td></tr>
                <tr><td style="padding-top: 24px;"><p style="margin: 0; font-size: 11px; letter-spacing: 2px; text-transform: uppercase; color: #e5ff21; font-weight: 600;">New Lead — Attention Guide</p></td></tr>
          ...
          <tr>
            <td style="background-color: #ffffff; padding: 40px; border-radius: 0 0 16px 16px;">
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 16px;">
                <tr>
                  <td style="padding: 12px 16px; background-color: #f5f5f0; border-radius: 10px;">
                    <p style="margin: 0 0 2px; font-size: 11px; letter-spacing: 1px; text-transform: uppercase; color: #999; font-weight: 600;">Biggest challenge</p>
                    <p style="margin: 0; font-size: 14px; color: #555;">${escapeHtml(biggestChallenge)}</p>
                  </td>
                </tr>
              </table>` : ''}
              ${wantsConsultation ? `
              ...
              <p style="margin: 0; font-size: 14px; color: #555;">Downloaded the Attention is New Gold guide from <a href="https://smpl.as/attention-guide" style="color: #141416;">smpl.as/attention-guide</a></p>
...
export async function POST(request: NextRequest) {
  try {
    const { name, email, company, biggestChallenge, wantsConsultation, attribution } = await request.json()
    ...
    await transporter.sendMail({
      ...
      to: email,
      subject: `Your Attention guide is ready, ${name.split(' ')[0]}!`,
    ...
    fetch('https://go.smpl.as/api/crm/email/log', {
      ...
      body: JSON.stringify({ template: 'attention_guide_user_delivery', to: email }),
    ...
    await transporter.sendMail({
      ...
      replyTo: email,
      subject: `${wantsConsultation ? '🔥 ' : ''}New lead: ${name}${company ? ` (${company})` : ''} downloaded Attention Guide${wantsConsultation ? ' — WANTS CONSULTATION' : ''}`,
      html: notificationEmailHtml(name, email, company, biggestChallenge, wantsConsultation),
    })
    fetch('https://go.smpl.as/api/crm/email/log', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({ template: 'attention_guide_team_notification', to: ['andreas@smpl.as', 'mike@smpl.as'] }),
    }).catch(() => {}) // never block send on logging

    // Sync to CRM
    const descParts = ['Downloaded Attention is New Gold Guide']
    if (wantsConsultation) descParts.push('Wants a free consultation')
    if (biggestChallenge) descParts.push(`Challenge: ${biggestChallenge}`)
    ...
    await syncToCrm({
      ...
      source: 'attention-guide',
      lead_magnet: 'attention-guide',
    ...
    console.error('Attention guide form error:', error)
    return NextResponse.json(
